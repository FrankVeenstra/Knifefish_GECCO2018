...
def initialPopulationEvaluation(pop, ser, data, hof):
    for i in range(populationSize):
        ser.write(pop[i].genome) # writing genome to arduino
        time.sleep(0.5) # pause between sending genome        
        fit = int(ser.readline()) # waits for incoming line (fitness value)      
...
def continueEvolution(pop, hof, ax, data):
    ser.flush()
    time.sleep(0.5)
    ...
    for i in range(populationSize):
        ...
        for t in range(amountTrials):
            ...
            time.sleep(0.5)
            fit = ser.readline() # waits until line is received
            print 'received robot fitness: ', fit
            fit_t.append(fit)
            ser.flush()
...
def run(hof, data, ax, name):
    ...
    #    continueEvolution(pop, hof, ax1, data)
