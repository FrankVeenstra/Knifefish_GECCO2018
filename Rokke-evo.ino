...
byte wave[LENGTH]; // Storage for waveform to play back from
...
float evaluateIndividual(int gene[]) {
  ...
  for (int i = 0; i < LENGTH; i++) { // Step across wave table
    ...
    sum = 90 + (sum / (geneLength / 2)) + 0.5; //0.5 added to round off correctly when converting to byte below
    wave[i] = (byte)sum;
  }

  //*3* Play back movement on robot from wavetable and sum up the sensor readings
  for (int i = 0; i < LENGTH - (amountServos * stepSize); i += stepSize) { // Step across wave table
    for (int n = 0; n < amountServos; n++) {
      int servoPos = wave[i + (stepSize * amountServos)];
